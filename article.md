...
- fail spuriously — это ситуация, когда мы без видимой причины говорим о том, что операция неуспешная. Похоже на spurious wakeup.
...
- Спинлок — это повторение атомарной операции в цикле, пока она не будет успешно выполнена.
...
Я написал небольшой lock-free thread-safe unbounded stack (чуть ниже разберу, что именно это означает). Данный код никоим образом не являвяется production-ready решением и даже близко не стоит с ним, а просто демонстрирует необходимость CAS операций. 
...
Также часто имеет место ABA problem, но предпочту не рассказывать о ней в данной статье.

## Модель памяти

### Отношение happens-before
Пока мы живём, то всегда совершаем какие-то действия и часто у этих действий есть определённый порядок. Например, мы сначала одеваемся, а потом уже выходим на улицу. Конечно, можно было бы сделать и наоборот, но мы любим делать именно так.
...
### Пример
Если вы поняли всё то, что было описано выше, то теперь вам предстоит пройти ещё одно испытание.

Пока я ко всем атомарным операциям добавлю ещё один параметр `std::memory_order::relaxed`. Постарайтесь понять, что именно делает код приведённый ниже.
...
По существу эта программа атомарно увеличивает два счётчика на единицу, сначала `x`, а потом `y`. Второй поток проверяет, всегда ли мы загрузим `x` больший, чем `y`. Если нет, то мы завершим исполненеие и напечатаем на экран когда мы завершили исполнение.
...
Думаю, что здесь всё стало на свои места и теперь более очевиден момент с перестановкой операций. Но вообще этот код содержит UB, поэтому я бы сказал, что это лишь иллюстрация, для наглядной демонстрации memory_order. При этом UB можно избежать, если `x` был бы атомиком. И если делать его атомиком, то на все операции (store/load) надо вешать memory_order_relaxed.
...
А что если мы хотим делать не только атомарные указатели, инты и так далее, но и сделать атомарную структуру... например, такую:
```cpp
struct Person {
    std::string username;
    std::string address;
...
```
Заранее скажу, что делать такое — плохая идея. Почему? Ну, потому что не всё то, что, сделано через `std::atomic<T>` будет атомарным.

Как это узнать? Можно использовать метод `is_lock_free`. 
...
А как же тогда будет работать наш атомик, если он не `lock-free`? Вообще это implementation defined, но чаще всего он будет реализован через мьютекс.

Я бы не рекомендовал делать атомики из чего-то размером больше, чем 8 байт. 