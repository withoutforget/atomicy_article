...
Вроде бы всё очевидно, но `compare_exchange` выглядит пугающе. При этом в `C++` их целых два: `std::atomic<T>::compare_exchange_weak, std::atomic<T>::compare_exchange_strong`. Грубо говоря, функция `strong` возвращает `false` только если гарантированно значения различны, а `weak` может вернуть `false` для одинаковых значений. Называется это `fail spuriously` (неожиданно провалиться). 
...
Выше написан код с `seq_cst` memory order, что позволяет синхронизировать значения `x` и `y` между собой. Однако давайте перепишем код на `acquire/release` memory order и проанализируем возможные переупорядочивания операций.
