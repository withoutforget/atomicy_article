...
- Убрать примеры с UB 
- Упомянуть ABA и как решают прикинуть
- Формализовать определения memory_order
- Уточнить немного как работает когерентность и как обеспечивается атомарность с lock 
- Рассказать про атомики от своих структур и тд, как они могут быть реализованы через мьютексы
- Дописать о учебных примерах, что они точно учебные
- Подправить моменты и писать точно
- Более жизненные примеры
...
- L1 — Кэш каждого ядра, размер 32Kb. Получение данных оттуда может занимать до 5 тактов.
- L2 — Кэш для одного ядра, но более медленный чем L1, размер 256Kb. Получение данных будет занимать в районе 20 тактов.
- L3 — Кэш общий для всех ядер, размер 2Mb. Получение данных может занимать порядка 50-100 тактов.

А чтение данных из RAM может занимать более двух сотен тактов.

Цифры могут быть, по сути, любыми, взял значения для примера, но масштаб +/- отображает.

Кэш-линия — блоки в кэшах. Чтобы эффективно доставать значения из кэшей придумали кэш-линии. Они обычно размером 64-128 байт, чтобы за раз доставать много информации.
...
- (M) Modified — состояние изменено только в этом кэше, данные отличаются от данных в RAM
- (E) Exclusive — состояние есть только в этом кэше и совпадает с RAM
- (S) Shared — состояние есть и в других кэшах и совпадает с RAM
- (I) Invalid — невалидное состояние кэш-линии
...
Поток 2: Атомарно прибавляет переменной a единицу.
    ...
    Кеш линия 2 = M (пред. S) (Состояние только в этой кеш-линии)
...
Поток 1: Видит, что кэш-линия невалидная и идёт загружать актуальную версию.
    ...
    Кеш линия 2 = S (пред. M) (Значение есть в нескольких кеш-линиях и совпадает)
